...
const Header = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center mb-2 sm:mb-4 animate-fade-in circuit-background min-h-[75vh] flex justify-center px-4">
      {/* Logo Section */}
      <motion.div 
        ...
        <a href="https://www.mastergrowbot.com" className="block">
          <div className="relative bg-card/80 p-3 sm:p-6 rounded-full backdrop-blur-xl ring-1 ring-white/10 hover:ring-accent/30 transition-all duration-500 shadow-2xl">
            <img 
              ...
              className={`transform group-hover:scale-105 transition-transform duration-500 ${isMobile ? 'w-16 h-16' : 'w-20 h-20 sm:w-28 sm:h-28'}`}
              loading="lazy"
              width={112} 
              height={112}
            />
      ...
      <div className="max-w-4xl mx-auto text-center px-2 sm:px-4">
        ...
        >
            ...
            >
              ...
              <div className="flex items-center space-x-1">
                {[...Array(5)].map((_, index) => (
                  <Star 
                    key={index} 
                    className={`fill-gold text-gold ${isMobile ? 'w-4 h-4' : 'w-5 h-5 sm:w-6 sm:h-6'}`}
                  />
                ))}
              </div>
            </motion.div>
          ...
          >
            <Link to="/quiz" className="inline-block">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className={`relative overflow-hidden font-semibold text-black rounded-xl transition-all duration-300 flex items-center justify-center group shadow-2xl ${isMobile ? 'text-base py-3 px-6 min-w-[240px]' : 'text-lg py-4 px-8 min-w-[280px]'}`}
                style={{
          ...
          {/* Footer Links */}
          <motion.div 
            className={`pt-4 sm:pt-6 text-muted-foreground ${isMobile ? 'text-xs' : 'text-sm'}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
